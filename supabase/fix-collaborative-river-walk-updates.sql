-- Fix RLS policy on river_walks table to allow collaborative updates
-- The current UPDATE policy only allows owners, blocking collaborators with editor permissions

-- Drop the existing restrictive UPDATE policy
DROP POLICY IF EXISTS "Users can update their own river walks" ON river_walks;

-- Create new UPDATE policy that allows both owners AND collaborators with editor role
