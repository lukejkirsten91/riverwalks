...
-- Add admin_role column to user metadata (if not exists)
-- Note: raw_user_meta_data usually already exists in Supabase
...
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'vouchers') THEN
